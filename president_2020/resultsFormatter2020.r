...
for (s in unique(elecData$state_po)){
  fname = paste0("2020", s, "Results.rds")
  f = readRDS(fname) %>%
...
saveRDS(rd2020, "2020ResultsBF.rds")
...
rd2020Filt = rd2020 %>%
  ...
  mutate(County = str_to_title(tolower(County)),
         Candidate=recode(Candidate, "DONALD J TRUMP"="Trump", "JOSEPH R BIDEN"="Biden"),
         BF.p = round(BF.p, 4),
         Mu = round(Mu, 4),
         Mu.SE = round(Mu.SE, 4),
         Sigma = round(Sigma, 4),
...
saveRDS(rd2020Filt, "fullFlags2020.rds")
write.csv(rd2020Filt, "fullFlags2020.csv", row.names=F)


# get cross-tabs
rd2020ContBF = rd2020 %>% # 3-way contingency table after bonferroni
...
ftable(addmargins(xtabs(~ GOF + Fitted + Observed, data = rd2020ContBF)))
